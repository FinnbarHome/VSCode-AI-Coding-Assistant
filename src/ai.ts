...
export async function getAIResponse(prompt: string): Promise<string> {
    try {
        ...
        // Create system message for regular analysis
        const systemMessage = `You are a strict AI code reviewer. Your response **must be structured into exactly 10 sections** using the format below:
        ...
        const fileName = `response-${new Date().toISOString().replace(/:/g, '-')}.txt`;
        const filePath = path.join(responsesDir, fileName);
